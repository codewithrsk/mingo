  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1">
        {filteredChatData.map((chat, idx) => (
          <>
            ...
            {/* <div
              key={idx}
              className={`chat ${chat.receiverId !== selectedFriend._id ? "chat-start" : "chat-end"}`}
            >
              <div className="chat-header text-xs text-base-content/40 mb-0.5">
                {chat.senderId !== sender._id ? receiver?.fullName : "You"}
              </div>
              <div
                className={`chat-bubble text-sm ${chat.senderId === sender._id ? "chat-bubble-primary" : ""}`}
              >
                {chat.message}
              </div>
      ...
      <div className="px-4 py-3 bg-base-100 border-t border-base-300 flex items-end gap-2 shrink-0">
        <button className="btn btn-ghost btn-sm btn-circle text-xl shrink-0">
          😊
        </button>
        <textarea
